...
```text CREATE OR REPLACE FUNCTION refresh_user_view() RETURNS VOID AS $$ BEGIN REFRESH MATERIALIZED VIEW CONCURRENTLY user_materialized_view; END; $$ LANGUAGE plpgsql SECURITY DEFINER; ``` 

Secure the function by revoking unnecessary access:

```sql REVOKE ALL ON FUNCTION refresh_user_view() FROM PUBLIC; GRANT EXECUTE ON FUNCTION refresh_user_view() TO admin_user; ``` 
...
```sql SELECT cron.schedule_in_database( 'refresh_user_view_daily', -- job_name '00 17 * * *', -- schedule: every day at 17:00 'SELECT refresh_user_view();', -- SQL command to run 'user_database' -- target database ); ``` 
...
```sql SELECT * FROM cron.job_run_details WHERE jobid = (SELECT jobid FROM cron.job WHERE jobname = 'refresh_user_view_daily'); ``` 

## Step 5: Test the Setup

Manually trigger the job to verify it works as expected:

```sql SELECT refresh_user_view(); ``` 
